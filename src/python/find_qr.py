...
def approxedQuadrR(contour):
    l = 0.1
    r = cv2.arcLength(contour, True) * 0.1
    flag = False
    while r - l > 0.1:
        m = (l + r) / 2
    ...
    return cv2.approxPolyDP(contour, r, True), r
...
def get_colors(contours, hierarchy):
    """
    Возвращает список 0/1 для каждого контура (цвет = parity(depth)).
    hierarchy: то, что вернул cv2.findContours (может быть None).
    """
    if hierarchy is None:
        return []
    h = hierarchy[0]
    parents = h[:, 3].astype(np.int32)
    n = len(parents)
    ...
    unknown = depths == -1
    while unknown.any():
        parent_known = (parents != -1) & (depths[parents] != -1)
        to_set = unknown & parent_known
        if not to_set.any():
            depths[unknown] = 0
            break
        depths[to_set] = depths[parents[to_set]] + 1
        unknown = depths == -1

    colors = (depths % 2).astype(int)
    return colors.tolist()


def get_colors_old(contours, hierarchy):
    # return get_colors_numpy(hierarchy)
...
def isQuadr_iou(contour):
    if cv2.contourArea(contour) == 0: return False
    return contour_iou(approxQuadr(contour), contour) >= 0.7

def isQuadr(contour):
    # return isQuadr_iou(contour)
    if cv2.contourArea(contour) == 0: return False
    app, d = approxedQuadrR(contour)
    return d <= 0.1 * cv2.arcLength(app, True)
...
def valid_combinations(search):
    ans = []
    trashold = 1
    if len(search) <= 3: trashold = 1.5
    
    mods = [max(np.linalg.norm(s[i] - s[(i + 1) % 4]) for i in range(len(s))) for s in search]
    all_comb = list(itertools.combinations([i for i in range(len(search))], 3))
    
    for c in all_comb:
        md = [mods[i] for i in c]
        if (max(md) - min(md)) / min(md) < trashold:
            ans.append(c)
...
def disp_evristic_sort(search, search_areas):
    ...
    all_search_sort = valid_combinations(search)
    ...
    # mods = [max(np.linalg.norm(s[i] - s[(i + 1) % 4]) for i in range(len(s))) for s in all_search[0]]
    
    # with open("ts.txt", "a") as f:
    #     f.write(f"{(max(mods) - min(mods)) / min(mods)}\n")
...
def find(img):
    ...
    # show_points(img, np.concatenate(all_search))
...
def recognize_distribution_(img, p1, p2, p3, p4, H = None):
    def get_pixel(p):
        if H is not None:
            p = H.predict(p)
    ...
    cur = Curve(p1, p2, p3, p4)
    print(cur._type)
    
    dx = np.sign(cur.dist(p3, p2))
    
    points = [cur.move(p2, dx * i) for i in range(int(abs(cur.dist(p2, p3))))]
    ...
    # print([abs(cur.dist(bord[i + 1], bord[i])) for i in range(len(bord) - 1)])
    # print(trashold)
    # print(cur)
    # for p in bord:
    #     print_as_point(p)
    
    # show_points(img, bord, 2)
...
def recognize_search_distribution_(img, p1, p2, H = None):
    def get_pixel(p):
        try:
            if H is not None:
                p = H.predict(p)
    ...
    cur = Curve(p1, (p1 + p2) / 2, p2)
    
    dx = 0.5 * np.sign(cur.dist(p2, p1))
    
    all_dist = 2 * int(abs(cur.dist(p1, p2)))
    
    # print(all_dist, (-all_dist // 7, all_dist * 8 // 7))
    
    points = [cur.move(p1, dx * i) for i in range(-all_dist // 7, all_dist * 8 // 7)]
    points = [np.array([int(i[0]), int(i[1])]) for i in points]
    ...
    trashold = max(abs(cur.dist(bord[i + 1], bord[i])) for i in range(len(bord) - 1)) * 0.1
...
def recognize_distribution(img, search, H = None):
    pattern1 = recognize_distribution_(img, (search[0] + search[3] * 13) / 14,
                                            ...
                                            (search[4] + search[7] * 13) / 14,
                                            (search[5] + search[6] * 13) / 14, H)
    
    pattern2 = recognize_distribution_(img, (search[0] + search[1] * 13) / 14,
                                            (search[3] + search[2] * 13) / 14,
                                            (search[12] + search[13] * 13) / 14,
                                            (search[15] + search[14] * 13) / 14, H)
...
def recognize_search_distribution(img, search, H = None):
    pattern1 = recognize_search_distribution_(img, (search[0] + search[3]) / 2,
                                                   (search[1] + search[2]) / 2, H)
    pattern2 = recognize_search_distribution_(img, (search[4] + search[7]) / 2,
                                                   (search[5] + search[6]) / 2, H)
                                            
    
    pattern3 = recognize_search_distribution_(img, (search[0] + search[1]) / 2,
                                                   (search[3] + search[2]) / 2, H)
    pattern4 = recognize_search_distribution_(img, (search[12] + search[13]) / 2,
                                                   (search[15] + search[14]) / 2, H)
    
    from interpolation_correct import Curve, Quintic
    
    def comp_pattern(pattern, p1, p2):
        if len(pattern) != 6:
            # show_points(img, pattern)
            return None
        
        cur = Curve(p1, (p1 + p2) / 2, p2)
        
        x = [0, 1, 2, 5, 6, 7]
        y = [cur.dist(p, p1) for p in pattern]
        qu = Quintic(x, y)
        pattern = pattern[:3] + [cur.move(p1, qu.predict(3)), cur.move(p1, qu.predict(4))] + pattern[3:]
        return pattern
    
    pattern1 = comp_pattern(pattern1, (search[0] + search[3]) / 2,
                                      (search[1] + search[2]) / 2)
    pattern2 = comp_pattern(pattern2, (search[4] + search[7]) / 2,
                                      (search[5] + search[6]) / 2)
                                            
    
    pattern3 = comp_pattern(pattern3, (search[0] + search[1]) / 2,
                                      (search[3] + search[2]) / 2)
    pattern4 = comp_pattern(pattern4, (search[12] + search[13]) / 2,
                                      (search[15] + search[14]) / 2)
    

    # show_points(img, pattern1 + pattern2 + pattern3 + pattern4, 3)
...
def zxing_find(img):
    ...
    cv2.imwrite("zxing-cpp_finder/input.png", img_arr)
...
def find_qr_zxing(orig, zxing_qr=None):

    if zxing_qr is None:
        zxing_qr = zxing_find(orig)

    prets = zxing_qr
    
    # img = cv2.imread("zxing-cpp_finder/output.png")
    ...
    for pret in prets:
        ...
        if len(pret[2]):
            ...
            # show_points(binary, np.concatenate(np.concatenate(contours)), 1)
            
            for lvl in range(0, 3):
                search_prets = find_search(contours, hierarchy, colors, lvl)
                # print(lvl, search_prets)
                if len(search_prets):
...
if __name__ == "__main__":
    # img = cv2.imread("orig_bynarized1.png")
    # img = cv2.imread("corrected.png")
    # img = cv2.imread("data/v2/M/big/img0095_v2_M_big_d2.png")
    # img = cv2.imread("data/v2/Q/big/img0155_v2_Q_big_d1.png")
    # img = cv2.imread("data/v2/H/small/img0035_v2_H_small_d2.png")
    # img = cv2.imread("data/v2/M/big/img0114_v2_M_big_d2.png")
    # img = cv2.imread("data/v2/H/small/img0035_v2_H_small_d2.png")
    # img = cv2.imread("data_orig/v2/H/small/img0034_v2_H_small_d2.png")
    img = Image.open("data2/test/150.png")
    ...
    # img = binarizer.binarize_zxing_cpp(img)
