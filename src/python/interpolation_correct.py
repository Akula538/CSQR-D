...
class Curve():
    ...
    def __init__(self, p1, p2, p3, p4 = None):
        ...
        def circle_metric(points):
            
            def _scale_value(points):
                pts = np.asarray(points)
                d = np.sqrt(((pts[np.newaxis,:,:] - pts[:,np.newaxis,:])**2).sum(axis=2))
                n = len(pts)
                if n <= 1:
                    return 1.0
                return d.sum()/(n*(n-1))
            ...
            xc, yc = -a/2, -b_coef/2
            R = np.sqrt((a*a + b_coef*b_coef)/4 - c)
            r = np.sqrt((x-xc)**2 + (y-yc)**2)
            d = np.abs(r - R)
            M_rms = np.sqrt(np.mean(d**2)) / _scale_value(points)
            M_max = np.max(d) / _scale_value(points)
            return {'xc':xc, 'yc':yc, 'R':R, 'M_rms':M_rms, 'M_max':M_max, 'd':d}
        
        if p4 is not None and circle_metric(np.array([p1, p2, p3, p4]))["M_max"] > 0.015:
            try:
                self._type = "parabola"
                
                self.A, self.B, self.C = math_base.fit_parabola_from4([p1, p2, p3, p4])
                return
            except: pass
    ...
    def I__(self, t):
        r_ = 2 * self.C * t + self.B
        return (r_@self.C*np.linalg.norm(r_)*np.linalg.norm(self.C) + cross(self.C, self.B) ** 2 * np.log(np.linalg.norm(r_)*np.linalg.norm(self.C) + r_@self.C)) / (4 * np.linalg.norm(self.C) ** 3)
    
    def p2t__(self, p):
        t = cross(p - self.A, self.C) / cross(self.B, self.C)
        
        def f(t):
            return np.linalg.norm(self.A + self.B * t + self.C * t * t - p)
        eps = 1e-6
        # print("====")
        t_ = (f(t + eps) - f(t)) / eps
        i = 10
        while f(t) > 0.3 and i > 0:
            i -= 1
            # print(t, t_, f(t))
            t = t - f(t) / t_
            # t = t - t_ * 0.00001
    ...
    def dist(self, p1, p2):
        if self._type == "parabola":
                      
            return self.I__(self.p2t__(p1)) - self.I__(self.p2t__(p2))
    ...
    def move(self, p, d):
        if self._type == "parabola":
            ...
            y = self.p2t__(p)
            d0 = self.I__(self.p2t__(p))
            eps = 1e-3
            while True:
                I = self.I__(y) - d0 - d
                # print(I, y, np.linalg.norm(2 * self.C * y + self.B), end = ' ')
                # print(self.C, self.B)
                if abs(I) < eps: break
                y -= I / np.linalg.norm(2 * self.C * y + self.B)
            
            return self.C * y * y + self.B * y + self.A
    ...
    def __str__(self):
        ...
        elif self._type == "parabola":
            return f"({self.A[0]}+{self.B[0]}t+{self.C[0]}t^2, {self.A[1]}+{self.B[1]}t+{self.C[1]}t^2)"
...
def CurveIntersection(cur1, cur2):
    ...
    raise ValueError("Intersection is not defended for this Curve type")
...
def recover_pattern_if_need(points, QRsize, pattern = [None, None], search_pattern = [None, None, None, None]):
    pattern = copy.deepcopy(pattern)
    if pattern[0] is None:
        cur1 = Curve(points[3], points[2], points[6])
        cub1 = constructCubic(cur1, points[3], [points[3], points[2], points[7], points[6]], [0, 7, QRsize - 7, QRsize])
        pattern[0] = []
        for i in range(8, QRsize - 7):
            pattern[0].append(cur1.move(points[3], cub1.predict(i)))
            
    if pattern[1] is None:
        cur4 = Curve(points[1], points[2], points[14])
        cub4 = constructCubic(cur4, points[1], [points[1], points[2], points[13], points[14]], [0, 7, QRsize - 7, QRsize])
        pattern[1] = []
        for i in range(8, QRsize - 7):
            pattern[1].append(cur4.move(points[1], cub4.predict(i)))
            
    if search_pattern[0] is None:
        search_pattern[0] = [(points[3] * (7 - i) + points[2] * i) / 7 for i in range(8)]
        
    if search_pattern[1] is None:
        search_pattern[1] = [(points[7] * (7 - i) + points[6] * i) / 7 for i in range(8)]
        
    if search_pattern[2] is None:
        search_pattern[2] = [(points[1] * (7 - i) + points[2] * i) / 7 for i in range(8)]
        
    if search_pattern[3] is None:
        search_pattern[3] = [(points[13] * (7 - i) + points[14] * i) / 7 for i in range(8)]
    
    pattern[0] = search_pattern[0] + pattern[0] + search_pattern[1]    
    pattern[1] = search_pattern[2] + pattern[1] + search_pattern[3]    
...
def interpolation_correct_homo(img, points, pattern = [None, None], QRsize = 45, square_size = 10, fract = 1, persp = None):
    ...
    if pattern[0] is not None:
        try:
            ...
            p = [points[3], points[2], points[7], points[6]]
    ...
    if pattern[0] is None:
        ...
        dist = [cub1.predict(i) for i in range(0, QRsize + 1)]
    ...
    if pattern[1] is None:
        ...
        dist = [cub4.predict(i) for i in range(0, QRsize + 1)]
    ...
    # blurred = cv2.GaussianBlur(blurred, (0, 0), 1)
